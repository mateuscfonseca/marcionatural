...
import type { UserEntry, TimelineEntry, EntryReport } from '@/types';
...
const props = defineProps<{
  modelValue: boolean;
  entry: UserEntry | TimelineEntry | null;
  reports?: EntryReport[];
  hasReported?: boolean;
...
<template>
  ...
  >
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="p-3 bg-gray-50 rounded-lg">
          ...
          <span
            ...
            :class="entry?.is_invalidated ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'"
          >
            {{ entry?.is_invalidated ? '❌ Invalidada' : '✅ Válida' }}
          </span>
        </div>
      </div>
      ...
      <div v-if="canReport" class="border-t pt-4">
        <button
          @click="handleReport"
          class="w-full py-3 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors flex items-center justify-center gap-2 cursor-pointer"
        >
          <span>🚩</span>
  ...
  <PhotoModal
    ...
    :model-value="showPhotoModal"
    :src="entry.photo_url"
    :alt="entry.description"
    @update:model-value="showPhotoModal = $event"
  />
</template>
